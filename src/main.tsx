...
// VERSÃO DA APP - Sempre que fizeres uma grande alteração, muda este número para acionar o aviso!
const APP_BUILD_ID = "v1.0.1";
...
export default function App() {
  ...
  // Verifica atualizações no servidor de 20 em 20 segundos
  useEffect(() => {
    const checkVersion = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // Se o HTML do servidor mudou, significa que há nova publicação na Vercel
  ...
  return (
    <div className="min-h-screen bg-[#080808] text-neutral-100 flex flex-col antialiased selection:bg-amber-500 selection:text-black">
      ...
      {/* IDENTIFICADOR DE VERSÃO INVISÍVEL PARA O SERVIDOR */}
      <div id="app-build-id" data-version={APP_BUILD_ID} className="hidden" />

      {/* BANNER DE ATUALIZAÇÃO NO TOPO ABSOLUTO */}
      {updateAvailable && (
        <div className="w-full bg-gradient-to-r from-amber-500 via-amber-400 to-amber-500 text-black py-2.5 px-4 flex items-center justify-between shadow-xl z-50 border-b border-black/10 animate-pulse">
